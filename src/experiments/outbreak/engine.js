import {
  TICKS_PER_DAY, FENCE_RADIUS_MULT,
...
// ─── DEBUG STATS SHAPE ───────────────────────────────────────────────────────
// Two objects: `debugStats` (resets each tick for per-tick readouts) and
// `cumStats` (cumulative run totals for verification).

function blankStats() {
  return {
    // Brownian motion (cumulative)
    wallBounces:     0,
    dotDotBounces:   0,
    fenceBounces:    0,
    ...
    // R0 tracking (cumulative)
    resolvedIndexCases:       0,
...
function effectiveP(config, iMasked, receiverMasked, receiverState) {
  let p = config.p
...
export function createSimState(config) {
  ...
  const dots = []
  for (let i = 0; i < N; i++) {
    const x = dotRadius + Math.random() * (canvasW - 2 * dotRadius)
    const y = dotRadius + Math.random() * (canvasH - 2 * dotRadius)
    ...
    dots.push({
      ...
      hasMask:            Math.random() < mwPct,
      isCompliant:        false,
      fenceId:            null,
      resolvedInfections: 0,  // how many S/R/V this dot successfully infected
    })
  }

  // Quarantine for initial I-dots
  const fences = []
  let nextFenceId = 0
  if (qp) {
    for (const dot of dots) {
      if (dot.state === 'I' && Math.random() < qcPct) {
        ...
        fences.push({ id: nextFenceId++, dotId: dot.id,
                      x: dot.x, y: dot.y,
                      radius: FENCE_RADIUS_MULT * dotRadius })
      }
    }
  }
  ...
  return {
    ...
    history:               [{ day: 0, ...countStates(dots), totalInfections: numInfected }],
    newInfectionsThisTick: 0,
    newInfectionsToday:    0,
    consecutiveZeroDays:   0,
    totalInfections:       numInfected,
    endCondition:          null,  // null | 'no-i' | 'no-infectable' | 'fizzle' | 'max-days' | 'user-stop'
    cumStats:              blankStats(),
    // Per-tick physics stats (reset each tick by caller if needed)
    tickStats: {
      wallBounces:   0,
      dotDotBounces: 0,
      fenceBounces:  0,
    },
    // Speed snapshot (updated each tick)
...
export function tickSim(state, config) {
  ...
  // Reset per-tick bounce counters (physics writes into cumStats directly)
  ...
  applyPhysics(
    ...
    state.dotRadius, config.collisionRadiusMult, state.cumStats, config.brownianMotion
  )

  // 2. Transmission — single O(n²) pass over I×{S,R,V} pairs
  //    Collect infections first, apply after loop to avoid mid-loop state drift
  const infections = [] // array of dot IDs to transition S/R/V → E
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < n; j++) {
      ...
      // Within collision radius — attempt transmission
      const p_eff = effectiveP(config, a.hasMask, b.hasMask, bState)
      ...
      if (Math.random() < p_eff) {
        infections.push({ targetId: j, sourceId: i, receiverState: bState })
      }
    }
  }

  // Apply infections (avoid re-infecting a dot that was just infected this tick)
  ...
  for (const { targetId, sourceId, receiverState } of infections) {
    ...
    if (receiverState === 'S') { state.cumStats.siSuccesses++ }
    else if (receiverState === 'R') { state.cumStats.riSuccesses++ }
    else if (receiverState === 'V') { state.cumStats.viSuccesses++ }
  }

  // 3. State duration advancement + transitions (E→I, I→R/D)
  for (const dot of state.dots) {
    ...
    if (dot.state === 'E' && dot.ticksInState >= dot.stateDurationTicks) {
      ...
      // Quarantine coin toss
      if (config.qp && Math.random() < config.qcPct) {
        dot.isCompliant = true
        dot.vx = 0
        dot.vy = 0
        ...
        state.fences.push({
          ...
          dotId: dot.id,
          x: dot.x, y: dot.y,
          radius: FENCE_RADIUS_MULT * state.dotRadius,
        })
      ...
      // Remove fence if this dot had one
      if (dot.fenceId !== null) {
  ...
  // 4. Daily infection counter + fizzle tracking
  ...
  // Record one history snapshot per sim-day
  if (state.tick % TICKS_PER_DAY === 0) {
    const { S, E, I, R, D, V } = state.counts
    state.history.push({ day: state.tick / TICKS_PER_DAY, S, E, I, R, D, V, totalInfections: state.totalInfections })
  }

  // 6. Speed snapshot for debug overlay
  ...
  // 'no-infectable' omitted: with V=0 default it fires the moment S=0 (before I/E resolve).
  // The 'no-i' condition above handles natural termination correctly on its own.
